$(function(){
    ...
    $(".guest-checkout .btn-guest").click(function(e){
        ...
        if(!emailRegEx.test(email)){
            $(".section.guest-checkout input[name=email]")
                .closest('p').addClass('tooltip error').find('em').remove().end().append("<em>Please enter a valid email address</em>").end().trigger('mouseover').end()
                .focus()[0];
            $(window).scrollTop(0);
            return;
        }
        $(".section.guest-checkout input[name=email]").prop('readonly', true)
                .closest('p').removeClass('tooltip error').find('em').remove();
        $(this).closest('.btn-area').hide();
        $(".shipping .new.frm, .shipping .btn-area").show();
    })
                ...
                .on('click', '.select_payment_', function() {
                    ...
                    if( $wrapper.find("form.new").is(":visible")){
                        ...
                        Checkout.Payment.addNewCard(params, function(data){
                            ...
                            //$wrapper.find(".selector ul.cardlist").find("li").remove().end().append($li);
        ...
        init: function(){
            ...
            $("#sidebar")
                ...
                .on('click', ".payment-coupon button.btn-apply", function(){
                    ...
                    var couponInfo = {"apply_coupon":true, "coupon_code":code, "email": email};
    ...
    function renderShippingAddress(data){
        if(data.shipping_info.addresses[0]){
            var $shipping = Checkout.Shipping.$el, address = data.shipping_info.addresses[0];
            $shipping.find("input,select").each(function(){
                var $this = $(this), key = $this.attr('name');
                if(key=='fullname') key="full_name";
                if(address[key]){
                    $this.val(address[key]).trigger('change');
                }
            })
        }
    }
