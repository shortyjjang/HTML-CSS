...
jQuery(function($){
    ...
    if( $nav.hasClass('hover_submenu') ){
        ...
        $nav
            ...
            .on('mouseover focus', 'a[class^="mn"]', function(e){
                ...
                if( !$this.is(".mn-menu, .mn-lang, .mn-noti:last-child, .mn-msg, .mn-seller") ) return;
            ...
            .on('mouseleave', 'li.gnb', function(){
                var $this = $(this);
                clearTimeout(timer);
                if( !$this.find(".mn-menu, .mn-lang, .mn-noti:last-child, .mn-msg, .mn-seller").length ) return;
            ...
            .on('mouseover focus', 'li.gnb span.trick', function(){
                var $this = $(this);
                clearTimeout(timer);
                if( !$this.closest('li.gnb').find(".mn-menu, .mn-lang, .mn-noti:last-child, .mn-msg, .mn-seller").length ) return;
    ...
    $nav
        .find('.trick:not(.full)').css('top',$('#header.v2').height()+'px').end()
        ...
        .on('click', 'a[class^="mn"]', function(e){            
            if ( $(this).hasClass('mn-menu') || $(this).hasClass('mn-lang') || $(this).hasClass('mn-you') || $(this).hasClass('mn-seller') || $(this).hasClass('mn-noti') || $(this).hasClass('mn-msg')) {
                if( $nav.hasClass('hover_submenu') ){
                    return;
                }
                if( $(this).hasClass('mn-noti') && $(this).closest('.gnb').find('.feed-activity').length < 1){
                    return;
                }
...
$(function(){
    ...
    var $list = $headerFeatured.find("ul");
    var perRow = 15;
    var maxPage = Math.ceil($list.find("li").length/perRow);
    var page = $.cookie.get('fh_page')||1;
    if( page > maxPage) page = maxPage;

    function scroll(_page, noanim){
        page = _page;
        var index = Math.max(0, (page-1)*perRow - 1);
        var left = 0;
        if( page == maxPage){
            left = 0-$list.find("li").eq(-15).position().left;    
        }else{
            left = 0-$list.find("li:eq("+index+")").position().left;    
        }

        if(page==1){
            $headerFeatured.find('a.prev').addClass('disabled');
        }else{
            $headerFeatured.find('a.prev').removeClass('disabled');
        }
        if(page==maxPage){
            $headerFeatured.find('a.next').addClass('disabled');
        }else{
            $headerFeatured.find('a.next').removeClass('disabled');
        }
        ...
        $.cookie.set('fh_page', page)
        $.cookie.set('fh_left', left)
    ...
    $headerFeatured
        ...
        .find('ul li a')
            .click(function(e){
                ...
                try {track_event('Click Circle Menu', {'title':title, 'url':url});} catch(e){}
            })
        .end()

    if($headerFeatured[0]) scroll(page, true);

    // activity feed
    $(document.body).on('click', 'ul.activity .activity-merchantpost', function(event) {
        var $el = $(event.target);
        var $curr = $(event.currentTarget);
        if (!$el.is('a, img') &&
            $el.closest('span.items')[0] !== undefined &&
            $curr.attr('data-aid') !== ''
        ) {
            location.href = '/activity?feed_id=' + $curr.attr('data-aid');
        }
    });
})
