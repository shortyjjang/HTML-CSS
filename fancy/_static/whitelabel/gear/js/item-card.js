jQuery(function($) {
    $(document).delegate('.actions > .like', 'click', function(event) {
        ...
        if( !isNaN( parseInt($button.data("fancyd-count"))) ){
            ...
            if(fancydCount>0){
                $button.next('.fancyd_user').show();
            }else{
                $button.next('.fancyd_user').hide();
            }
        }
    ...
    $(document.body)
        .on('mouseover', '.itemListElement', function(){
            $(this).find("img[data-style]").each(function(){
                $(this).attr('style', $(this).data('style')).removeData('style');
            })
        })
