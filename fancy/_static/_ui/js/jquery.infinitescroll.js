...
(function (window, $, undefined) {
	...
	$.infinitescroll.defaults = {
		...
		currPage: 1,
		timestamp: 0,
		timestampSelector: 'a.btn-more',
		setWhenDone:false,
		no_featured_find: false,		
		infid: 0, //Instance ID
		isDuringAjax: false,
		isInvalidPage: false,
		isDestroyed: false,
        ...
        */
        ...
        _loadcallback: function infscr_loadcallback(box, data) {
            ...
            switch (result) {
                ...
                case 'append':
					var ts = $(data).find(opts.timestampSelector).attr('ts');
					if (ts != undefined && ts != null){
						opts.timestamp=ts;
						if(opts.isPaused){
							$(opts.navSelector).show();
							var href_url = $(data).find(opts.timestampSelector).attr('href');
							$(opts.timestampSelector).attr('href',href_url);
							$(opts.timestampSelector).attr('ts',ts);
						}
					}
					else{
						if(opts.no_featured_find)
							opts.timestamp=0;
						else		
							opts.timestamp=0;
        ...
        retrieve: function infscr_retrieve(pageNum) {
            ...
            var instance = this,
				...
				beginAjax = function infscr_ajax(opts) {
	                ...
	                instance._debug('heading into ajax', opts.timestamp);
	                instance._debug('heading into ajax', path);

	                // if we're dealing with a table we can't use DIVs
	                box = $(opts.contentSelector).is('table') ? $('<tbody/>') : $('<div/>');

	                desturl = path.join(opts.timestamp);
					
					if (opts.no_featured_find){
						desturl = desturl+"?featured_find=false";
					}
