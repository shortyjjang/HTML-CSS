...
function openLogin(){
    ...
    $("#popup_container")
        ...
        .find("[name=next_url]").val( location.href );
}
...
jQuery(function($) {
    ...
    */
    ...
    window.signup = {
        ...
        check_name : function(check_all) {
            ...
            if(check_all && name.length<1) {
                signup.show_error($(field), 'Name is required')
                return false;
            }
            if($(this).val().length>30) {
    ...
    signup.init_form_signup = function(form_signup, extraValidation, onSuccess, onFail){
        form_signup
    ...
    signup.init_form_signup(form_signup, null, function(response){
    ...
    signup.init_form_signup_detail = function(form_signup, form_signup_detail, extraParams, onSuccess){
        form_signup_detail
            .on('open', function(event) {
                event.preventDefault()
            ...
            .on('click', '.btn-signup', function(event) {
                ...
                $.post(https_post, param, function(response) {
                    if (response.status_code != undefined && response.status_code == 1) {
                        ...
                        onSuccess(activate_url, next_url);
                    } else if (response.status_code != undefined && response.status_code == 0) {
                        var msg = response.message;
                        var error = response.error;
    ...
    signup.init_form_signup_detail(form_signup, form_signup_detail, {}, function(activate_url, next_url){
        ...
        if(sns_type==='facebook') {
            ...
            $.post("/facebook_signup.json",param, function(json) {
                if (json.status_code === 1) {
                    // success
                    onCompleteSignup('gear_facebook');
