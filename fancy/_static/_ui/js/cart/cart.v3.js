jQuery(function($){
	...
	$('form.merchant_')
		...
		.on('submit', function(){
			...
			$this.find(':submit,.btn-wallet').attr('disabled', true);
		})
		// quantity
		...
		.on('click', 'a.remove-item_', function(event){
			...
			removeItem($this, data, function(json){
				...
				var itemCount = $form.find('.item_').length;
				...
				if(itemCount>1){
					$addback
						.find("a")
							.html($li.find("b.title").html()).attr('href', $li.find(".item a").attr('href'))
						.end()
		...
		.on('click', 'button.undo_', function(event){
			...
			var params = {
				seller_id    : $this.attr('data-sid'),
				quantity     : $this.attr('data-qty'),
				thing_id     : $this.attr('data-tid'),
				sale_item_id : $this.attr('data-siid'),
				user : window.owner
			};
	...
	$("div.saved-later")
		...
		.on('click', 'a.remove', function(event){
			...
			$.ajax({
				...
				success  : function(res){
					if (res.thing_id) {
						$this.closest(".cart-item-detail").find("li.add-back").hide();
						var $addback = $this.closest(".cart-item-detail").find("li.add-back._remove");
						if($this.attr('wishlist')){
							$addback = $this.closest(".cart-item-detail").find("li.add-back._wishlist");
	...
	// wanted list
	$('div.wants')
		.on('click', 'a.cart_', function(event){
			var $this = $(this), params, tname = $this.closest('li').find('b.figcaption').text();
			...
			$.jStorage.set('cart.notification', gettext('<b>{name}</b> has been moved to <b>Shopping Cart.</b>').replace('{name}', tname), {TTL:10000});

			addToCart($this, params, function(){ updateCart() && updateWants(); });
		})
		.on('click', 'a.del_', function(event){
			var $this = $(this), params;

			event.preventDefault();

			dontWant($this, $this.attr('data-rtid'), function($xml){ if ($xml.find('status_code').text() == 1) updateWants() });
		});

	function dontWant($btn, rtid, callback){
		$btn.addClass('disabled').attr('disabled', true);

		$.ajax({
			type : 'POST',
			url  : '/delete_want_tag.xml',
			data : {rtid:rtid, user : window.owner},
			dataType : 'xml',
	...
	function makeScrollable($el){
		...
		$el.find(".paging > small").html("Page 1 of "+maxPage);
		if(maxPage==1){
			$el.find(".paging").hide();
		}
		$el.find(".paging a.next").click(function(e){
			e.preventDefault();
			var page = $ul.attr("page")||1;
			if(page==maxPage) return;
			page++;
			var newLeft = (page-1)*966;
		...
		$el.find(".paging a.prev").click(function(e){
			e.preventDefault();
			var page = $ul.attr("page")||1;
			if(page==1) return;
			page--;
    ...
    var trackData = (function getTrackData() {
      try {
        ...
        var box = $item.find('.qty .select-boxes2');
      ...
      return [String(price * quantity), [{
		...
		if(Fancy.Cart){
			Fancy.Cart.removeItem({ITEMCODE: data.sale_item_cart_id})
		}
	...
	function updateCart(){
		...
		$btn = $cart.find('button:submit,.btn-wallet');
		...
		function updateSaleitem(item){
			var id=item.item_id, $row = $cart.find(".item_[id=item-"+id+"]"), $tpl = $cart.find("ul.cart-item-detail > script");
			...
			if(!$row.length){
				...
				$cart.find(".cart-item ul.cart-item-detail").prepend($row);
			}
		...
		function updateSaleitems(_items) {
	    	...
	    	$cart.find('.item_').each(function(){
				...
				Fancy.Cart.updateItem({ITEMCODE: $row.find('[data-sicid]').data('sicid'), QUANTITY: item.quantity})
			});
			...
			if(itemcount){
	            ...
	            $cart.find('.btn-wallet').addClass('hidden');
