$(function() {
    ...
    function initSlide($el, option){
        var $slide = $el.find(".itemSlide, .slideshow"), $list = $el.find("ul"), $li = $list.find("li"), itemWidth = $li.width()+20, paddingleft = parseInt($list.css('padding-left')), $prev = $el.find("a.prev"), $next = $el.find("a.next");
        var slideWidth = null;
        ...
        function getPrevLeft($el){
            preparePrev();
            var left = Math.abs(parseInt($list.css('left'))), nextLeft, width = $slide.width();
        ...
        function preparePrev(){
            var $li = $list.find("li"), left = Math.abs( parseInt($list.css('left') ) ), width = $slide.width(), itemWidth = $li.width()+20;
            if(slideWidth) width = slideWidth;
        ...
        function prepareNext(){
            var $li = $list.find("li"), left = Math.abs( parseInt($list.css('left') ) ), width = $slide.width(), listWidth = $li.length * ($li.width()+20), itemWidth = $li.width()+20, spare = listWidth - (left + width);
            if(slideWidth) width = slideWidth;
        ...
        function clearOld(){
            var $li = $list.find("li"), left = Math.abs( parseInt($list.css('left') ) ), itemWidth = $li.width()+20, $prev = $list.find("li[_next]:not(:in-viewport)"), prevCnt = $prev.length;
