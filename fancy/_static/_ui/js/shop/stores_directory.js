$(function(){
	...
	$('.abc').delegate('a','click',function(event){
		
		event.preventDefault();
		var url = location.pathname, index = $(this).attr('rel'), args = $.extend({}, location.args), query;

		var $el = $(".alphabet[data-index="+index+"]");		
		if($el[0]) $("html, body").animate({scrollTop: $el.offset().top-40},200);
	});
		...
		$.ajax({
			...
			success  : function(html){
				...
				(function(){
					...
					if(!$stream.find("dl.wrapper").length){
