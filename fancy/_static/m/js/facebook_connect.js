jQuery(function($) {
	...
	// add script
	$('<script type="text/javascript" src="https://connect.facebook.net/en_US/sdk.js" async></script>').appendTo('#fb-root');
			...
			if ($(xml).find("status_code").length>0 && $(xml).find("status_code").text()==1 && $(xml).find("url").length>0) {
                ...
                if (url.indexOf('/register') == 0) {
                    ...
                    if(window.simpleRegister && next_u=='/checkout') {
                        ...
                        location.href = url+(next_u?'?next='+encodeURIComponent(next_u):'');
                    }
                } else {
                    try {track_event('Complete Login', {'channel':'facebook', 'via':'mobile'});} catch(e){}
                    location.href = url;
                }
			}
			else if ($(xml).find("status_code").length>0 && $(xml).find("status_code").text()==1) {
				location.href = '/register'+(next_u?'?next='+encodeURIComponent(next_u):'');
			}
