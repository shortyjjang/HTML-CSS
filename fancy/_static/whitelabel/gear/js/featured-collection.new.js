$(function(){
    ...
    function enhance_images() {
        if($(window).width() > mSize) return;

        $('.collections .itemListElement .detail-overlay .figure-list a[data-image]').each(function(i,el) {
            $(el).find('img').css('background-image', 'url('+$(el).data('image')+')');
        })
    }
