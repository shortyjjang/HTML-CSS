...
(function(factory){
	...
	function uploader(options) {
		...
		if (typeof options == 'string') {
			cmd = options;
			(arg = [].slice.call(arguments)).shift();
		}
	...
	function Uploader(element, options) {
		...
		if (this.options.dragndrop) {
			this.$el.on('dragover', $.proxy(this, 'dragover'));
			this.$el.on('dragleave', $.proxy(this, 'dragleave'));
			this.$el.on('drop', $.proxy(this, 'drop'));
		} else {
			this.$el.on('mouseover mousemove', $.proxy(this, 'mousemove'));
			this.$file = $('<input type="file">')
				.prop('multiple',this.options.multiple)
				.css({position:'absolute',top:0,left:0,width:'10px',height:'10px',opacity:0})
				.on('change', $.proxy(this, 'change'));
		}
	...
	$.extend(Uploader.prototype, {
		...
		change : function(event) {
			this.files = this.$file[0].files || this.$file[0];
		...
		upload : function() {
			...
			if (this.supportHTML5) {
				...
				for (var i=0; i < this.files.length; i++) {
					this.formData.append(this.options.field, this.files[i]);
				}
			} else {
				if (!this.$frame) {
		...
		progress : function(event) {
			if (event.lengthComputable) {
				var percent = event.loaded * 100 / event.total, size = 0;
				for (var i=0; i < this.files.length; i++) {
		...
		addField : function(name, data) {
			if (this.formData) {
				...
				$('<input type="hidden" name="'+name+'">').val(data).appendTo(this.$form);
			}
		},
		enable : function() {
			this.enabled = true;
			this.$file.css('display', '');
		},
		disable : function() {
			this.enabled = false;
