                    ...
                    if ($url.length) {
                        if ($url.text().indexOf('/register') == 0) {
                            ...
                            if(window.simpleRegister && next_u=='/checkout') {
                                ...
                                location.href = $url.text()+(next_u?'?next='+encodeURIComponent(next_u):'');
                            }
                        } else {
                            try {track_event('Complete Login', {'channel':'twitter', 'via':'mobile'}); } catch(e) {}
						    location.href = $url.text();
                        }
					} else if(referrer) {
   						location.href = '/register?referrer='+(referrer)+(next_u?'&next='+encodeURIComponent(next_u):'');
					} else {
