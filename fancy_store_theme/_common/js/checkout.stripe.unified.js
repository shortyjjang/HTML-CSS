jQuery(function($){
	...
	View.Checkout = View.DataBinding.extend({
		events : {
			...
			'keydown input.text' : 'onFormKeyDown',
		},
		initialize : function(options) {
            ...
            if (globals.paypal_payment) {
                this.fetch({type:'PUT', data: globals.paypal_payment, update_credit_card: false});
            } else if (globals.convert_currency) {
                var data = { payment_gateway: 18 };
		...
		fetch : function(options) {
			...
			$forms = self.$(options.section||'.summary, .order-note, .gift-option, .order-coupon, .shipping, .shipping-type, .payment').addClass('loading');
			...
			options.success = function(collection, json) {
                ...
                if (globals.paypal_payment && !(json.payment_gateway == 9 && json.extra_info.paypal_payment_id && json.extra_info.paypal_payer_id)) {
				...
				var summary;
                if (globals.convert_currency) {
                    summary = {
                        'subtotal_price': json.subtotal_prices_converted, 'total_price': json.total_prices_converted,
                        'tax': json.sales_taxes_converted, 'shipping': json.shipping_costs_converted,
                        'coupon_amount': json.coupon_amount_converted, 'fancy_rebate': json.credit_amount_converted,
                        'fancy_gift_card': json.fancy_money_amount_converted
                    };
                } else {
                    summary = {
                        'subtotal_price': json.subtotal_prices, 'total_price': json.total_prices,
                        'tax': json.sales_taxes, 'shipping': json.shipping_costs,
                        'coupon_amount': json.coupon_amount, 'fancy_rebate': json.credit_amount,
                        'fancy_gift_card': json.fancy_money_amount
                    };
                    if (json.antavo && json.antavo.max_points > 0) {
                        summary['antavo_discount'] = json.antavo.max_discount;
                        summary['antavo_points'] = json.antavo.max_points;

                        var total_price = parseFloat(json.total_prices) - parseFloat(json.antavo.max_discount);
                        var reduce_fancy_money = parseFloat(json.antavo.reduce_fancy_money);
                        if (reduce_fancy_money > 0) {
                            total_price += reduce_fancy_money;
                            summary['fancy_gift_card'] = (parseFloat(json.fancy_money_amount) - reduce_fancy_money).toFixed(2);
                        }
                        summary['total_price'] = total_price.toFixed(2);
                    }
                }
				...
				// addreses
                if(loggedIn || globals.paypal_payment) {
                ...
                if (globals.paypal_payment) {
                    self.$el.find("#signup input[name=email]").val(json.extra_info.paypal_email).prop('readonly', true);
                }
		...
		render : function(model, repaint) {
			...
			if ('addresses' in changed) {
				...
				if (changed.addresses.length && $sel.length) {
		...
		onFormKeyDown : function(event) {
			if (event.keyCode === 13) { // enter input
				var $els = $('input.text:visible');
				var idx = $els.index(event.currentTarget) + 1;
				var $nextEl = $els.eq(idx);
				if ($nextEl[0]) {
					$nextEl.focus();
				}
			}
		...
		onSubmitOrder : function(event) {
			...
			function payment() {
                ...
                $('textarea.personalization').each(function (i, e) {
					var $el = $(e);
					if ($el.val().trim() !== '' && $el.data('sale-id')) {
						personalizations = personalizations || {};
						var itemKey = $el.attr('data-sale-id');
						if (!!$el.attr('data-option-id')) {
							itemKey += ('-' + $el.attr('data-option-id'))
						}
						personalizations[itemKey] = $el.val().trim();
					}
				}); 
                ...
                self.fetch({type:'PUT',data:data,
                    success:function(res) {
                        ...
                        var payment_data = {
                            payment_gateway: globals.convert_currency ? 18 : 6,
                            ordered_via:"storefront"
                        };
                        if (globals.paypal_payment) {
                            payment_data['payment_gateway'] = 9;
                            payment_data['paymentId'] = globals.paypal_payment.paymentId;
                            payment_data['payerId'] = globals.paypal_payment.payerId;
                        } else if( globals.userInfo) {
                        ...
                        if (self.model.get('summary').antavo_points) {
                            payment_data['antavo_points'] = self.model.get('summary').antavo_points;
                        }
                        if (globals.usesandbox) payment_data['usesandbox'] = 'true';
			            self.$('.summary, .order-note, .gift-option, .order-coupon, .shipping, .shipping-type, .payment').addClass('loading');
                        ...
                        $.ajax({
			                ...
			                self.$('.summary, .order-note, .gift-option, .order-coupon, .shipping, .shipping-type, .payment').removeClass('loading');
    						$btn.prop('disabled', false).removeClass('loading').text('Submit Order');
    						$payment.removeClass('loading');
    					});
                    },
