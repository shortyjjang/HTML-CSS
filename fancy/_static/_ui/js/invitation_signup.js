jQuery(function($){
    ...
    $('form button.sign').on('click',function(event){
	...
	function displayError(fieldSelector, msg) {
		...
		if ($error.length > 0) { 
			$error.css('display','inline-block').text(msg);
	...
	if(document.location.protocol=="https:"){		
		$.post(
			...
			if (response.status_code != undefined && response.status_code == 1) {
                ...
                /* we are not using captcha
			    else if(response.error.indexOf('captcha')>=0) {
				Recaptcha.reload();
		...
		$("<form id='signup_form' action='https://"+document.location.hostname+http_post+"' method='post' target='signup_iframe'></form>").appendTo(document.body);
		for(var k in param){
