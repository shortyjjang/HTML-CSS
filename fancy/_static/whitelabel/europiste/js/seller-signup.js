...
jQuery(function($) {
    $('#content.frm .step0 button#btn-signup-step0').click(function(event) {
        $('.frm.step0').addClass('step2').removeClass('step0');
    })
    ...
    $('#content.frm .step1 button#btn-signup-step1').click(function(event) {
        ...
        if( "username" in params ){
            if( !params['username'] ){
                alertify.alert(gettext("Please enter your username"));
                return;
            }
            if(params['username'].length<3 || params['username'].length>30) {
                alertify.alert(gettext("Username is too short or too long"));
                return;
            }
            // see common/util.js to change usernameRegEx
            if(!usernameRegEx.test($.trim(params["username"]))) {
                alertify.alert(gettext("Please enter a valid username"));
                return;
            }
        }
        if( "email" in params ){
        ...
        function goto_step0() {
            $('.frm.step1').addClass('step0').removeClass('step1');
...
jQuery(function($) {
    ...
    function get_signup_params() {
        ...
        if($form_signup_basic.find('input[name="username"]').length) {
            signup_params['username'] = $form_signup_basic.find('input[name="username"]').val()
        }
