...
( function( $ )
{
	...
	$.fn.sortableLists = function( options )
	{
        ...
        $.fn.readySortableListsItem = function() {
    		if ( setting.opener.active )
    		{
    			...
    			$( this ).find( 'li' ).each( function()
    			{
    				...
    				{
                        if(!li.find('.sortableListsOpener').length)  {
		...
		function endDrag( e )
		{
            ...
            state.rootEl.el.find('li.closeAfterDrag').each(function(i,li) {
                if($(li).find('#sortableListsHint').length) return;
                close($(li));
            });
            state.rootEl.el.find('li.closeAfterDrag').removeClass('closeAfterDrag');
			...
			cEl.el.animate( { left: offset.left - state.cEl.mL, top: offset.top - state.cEl.mT }, 250,
				function()  // complete callback
				{
					...
					if ( hintWrapperNode.length )
					{
    					...
    					opener.clone( true ).prependTo( hintWrapperNode.prev( 'div' ).first() );
					}
		...
		function showOnTop( e, oEl )
		{
			...
			else
			{
				...
				if ( state.oEl )
				{
					open( oEl, true ); // TODO:animation??? .children('ul,ol').css('display', 'block');
				}
		...
		function showOnTopPlus( e, oEl, outside )
		{
			...
			if(!state.isAllowed) {
                hint.css('display','none');
                hint.addClass('hidden');
                if(oEl && oEl.children(setting.listSelector).length <= oEl.children("."+setting.hintWrapperClass).length) {
                    close(oEl);
                }
            }
        }
		 ...
		 * @param li
		 */
		function open( li, while_dragging=false )
		{
			if(while_dragging && !li.hasClass( 'sortableListsOpen' )) {
                li.addClass('closeAfterDrag');
            }
