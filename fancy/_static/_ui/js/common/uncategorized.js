...
$(document).ready(function(){
	...
	if( /Safari/.test(navigator.userAgent) ){$('body').addClass('chrome');}
	...
	$('.group-gifts .btn-edit').each(function(){
		var tLeft = -($(this).find('.tooltip small').width()+14)/2
		$(this).find('.tooltip small').css('margin-left',tLeft+'px');
	});
});
...
jQuery(function($){
	...
	function adjustSidebarPosition(){
        if ($(window).width()<=$('.container').outerWidth()) {
            $('#header.v2').css('left',-$(window).scrollLeft()+'px').css('padding-right',$(window).scrollLeft()+'px');
            $container.find('.navigation, .thing-detail-container .summary').css('left',$('.container').offset().left-$(window).scrollLeft()+'px').css('margin-left','0');
            if($container.find('#header_summary').length) $container.find('#header_summary').css('left',$container.find('#content').offset().left-$(window).scrollLeft()+'px').css('margin-left','0');
            $container.find('.timeline_feature').css('left',$('.container').offset().left+($('.container').width()/2)+'px');
        } else {
            $('#header.v2').css('left','0px').css('padding-right','0px');
            $container.find('.navigation').css('left','50%').css('margin-left','-579px');
            $container.find('.thing-detail-container .summary').css('left','50%').css('margin-left',-$container.find('.thing-detail-container .summary').width()/2+'px');
            if($container.find('#header_summary').length) $container.find('#header_summary').css('left',$container.find('#content').offset().left+'px').css('margin-left','0');
            $container.find('.timeline_feature').css('left','50%').css('margin-left','299px');
        }
    }
...
jQuery(function($) {
    ...
    $('#content.activity-list').delegate('.paging.with-arrow a', 'click', function(event) {
        ...
        if($(this).hasClass('prev')) {
            if($current_item.hide().prev().show().prev().length<=0) {
                $(this).addClass('disabled');
            }
        } else if($(this).hasClass('next')) {
            if($current_item.hide().next().show().next().length<=0) {
                $(this).addClass('disabled');
            }
        }
    });
