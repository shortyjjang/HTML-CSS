$(function() {
    var newSlide = "newslide" in location.args;

    function getTr($el) {
        if ($el.prop("style") && $el.prop("style").transform) {
            var t = $el.prop("style").transform;
            return parseInt(t.match(/\((.*)px\)/)[1]);
        } else {
            return 0
        }
    }

    function newSlideInit() {
        var mWidth = 920;
        function initSlide($el, option) {
            var $slide = $el.find(".itemSlide, .slideshow"),
                $list = $el.find("ul"),
                $li = $list.find("li"),
                itemWidth = $list.find("li:first-child").width() + parseInt($list.css("margin-left")),
                $prev = $el.find("a.prev"),
                $next = $el.find("a.next");
            var slideWidth = null;
            ...
            function resetRecommend() {
                ...
                if ($(window).width() < mWidth) {
                    if (option.center) {
                        $slide.find(".itemList").scrollLeft(($li.length * itemWidth - $(window).width() + 20) / 2);
                    }
                    ...
                    if ($slide.width() > itemWidth * $li.length) {
                        ...
                        if (option.center) {
                            $slide.find(".itemList").prop("style").transform = "translateX(0px)";
                        }
                    } else {
                        if (option.center) {
                            $slide.find(".itemList").prop("style").transform = "translateX(" + -parseInt(($li.length * itemWidth - $(window).width() + 20) / 2) + "px)"
                        }
                        $prev.removeClass("disabled");
                        $next.removeClass("disabled");
                    }
            ...
            function getPrevLeft() {
                preparePrev();
                var left = Math.abs(getTr($list)),
                    nextLeft,
                    width = $slide.width();
            ...
            var proceedTransition = true;
            var transitionAmount = 5;
            $slide.find(".itemList").css("transition", "transform 0.25s linear");
            setInterval(function() {
                if (proceedTransition && $list[0]) {
                    var curr = getTr($slide.find(".itemList"));
                    $list.prop("style").transform = "translateX(" + (curr - transitionAmount) + "px)"
                }
            }, 1000);
            $el.on("click", "a.prev", function(e) {
                ...
                .on("mouseout", function() {
                    ...
                    $list.css("transition", "transform 1s linear")
                });
        }

        $.fn.recommendSlide = function(option) {
                ...
                if ($(window).width() < mWidth) {
                    ...
                    if ($slide.width() > itemWidth * $li.length) {
                        ...
                        if (option.center) {
                            $slide
                                .find(".itemList")
                                .css("left", -parseInt(($li.length * itemWidth - $(window).width() + 20) / 2) + "px");
                        }
                        $prev.removeClass("disabled");
                        $next.removeClass("disabled");
                    }
