function enable_scroll_handler() {
        var $sidebar = $("#sidebar");
        if($sidebar[0]){
            var prevScrollTop = 0;
            var isDown = true;
            ...
            $(window).on('scroll',function () {
                ...
                if ( scrollTop > sidebarOffsetTop - 57 && $("#content").height() > sidebarHeight ) {                
                    // 57 == gap + fancy top bar
                    // Now, reached point where gap + top of sidebar is going to get clipped
                    isDown = prevScrollTop < scrollTop;
                    var offsetTop = $sidebar.offset().top;
                    if( windowHeight < sidebarHeight+57 ){
                        if( !$sidebar.hasClass('fixed') ){
                            if( isDown && (scrollTop + windowHeight) > (offsetTop + sidebarHeight)){
                                $sidebar.addClass('fixed').css({'bottom':'1px','position':'','top':''});    
                            }
                            if( !isDown && offsetTop-57 > scrollTop ){
                                $sidebar.addClass('fixed').css({'top':'57px','position':'','bottom':''});           
                            }
                        }else{
                            /*
                            if( !isDown && $sidebar.hasClass('fixed') && parseInt($sidebar.css('bottom')) ){
                                $sidebar.removeClass('fixed').css({bottom:'', position:'absolute', 'top': offsetTop+'px'});
                            }
                            */
                            if( isDown && $sidebar.hasClass('fixed') && parseInt($sidebar.css('top')) ){
                                $sidebar.removeClass('fixed').css({bottom:'', position:'absolute', 'top': offsetTop+'px'});
                            }
                        }
                    }else{                  
                        $sidebar.addClass('fixed').css({'top':'57px','bottom':'','position':''});   
                    }
                    prevScrollTop = scrollTop;

                } else {
                    $sidebar.removeClass('fixed').css({'top':'','bottom':'','position':''});
                }
            });
        }
}

function disable_scroll_handler() {
  $(window).off("scroll");
}
...
$(function(){
    ...
    // sort option
    $('#sidebar .sort input[name=sort_by_price], ul.refine .sort select[name=sort]').change(function(event){
    ...
    // search by color
    $('#sidebar .color li').click(function(event){

        if( $(event.target).is("input") || $(this).is(".more") ) return;
        var color = $(this).find("input:checkbox").val(), url = location.pathname, args = $.extend({}, location.args), query;

        if(color && !$(this).hasClass("checked") ){
            args.c = color;
        } else {
            delete args.c;
        }
        ...
        // immediate shipping
    $('#sidebar .option input[name=is]').click(function(event){
    ...
    $('#sidebar .option input[name=ss]').click(function(event){
        if(this.checked){
            $('#sidebar .option fieldset').show();
            $('#sidebar input[name=ss]').parent().addClass("selected");
    ...
    $('#sidebar .option button.btn-check').click(function(event){
        filterBySamedayShipping();
    });
    ...
    function setOptions(params){
        ...
        if(params.c){           
            $('#sidebar .color li input[value='+params.c+']').parent().addClass("checked").end()[0].checked=true;
        }
        ...
        if(params.sort_by_price){
            if($('#sidebar .sort li input').length){
                $('#sidebar .sort li input[value='+params.sort_by_price+']').parent().addClass('selected').end()[0].checked=true;
            }
        }

        // set immediate shipping
        $('#sidebar #is').parent().removeClass('selected');
        if(params.is) {
            $('#sidebar input[name=is]').parent().addClass("selected").end()[0].checked=true;
        }
        
        // set sameday shipping
        $('#sidebar #sdd').parent().removeClass('selected');
        $('#sidebar .option fieldset').hide();
        if(params.ss) {
            $('#sidebar .option fieldset').show();
            $('#sidebar input[name=ss]').parent().addClass("selected").end()[0].checked=true;
        }
            ...
            if(isLists) {
                ...
                if($this.attr('sid')){
                    ...
                    if(isFollow){
                        ...
                        try{track_event('Unfollow Store', {seller_id:params.seller_id});}catch(e){}
                    }
                }
    ...
    $(".toggle_show_in_newest").click(function(e){
        ...
        $.ajax({
            ...
            data : {spid:spid, show_in_newest:show_in_newest},
            dataType : 'json',
            success  : function(json){
                // to do something?
                if (json.status_code) {
